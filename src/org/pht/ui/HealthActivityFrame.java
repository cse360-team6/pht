...
public class HealthActivityFrame extends ActivityFrame {
	...
	ok.addActionListener(new ActionListener() {
		...
		if (e.getSource() instanceof JButton) {
		    ...
		    if (((String) activities.getSelectedItem())
			    .compareTo(healthInformation[0]) == 0) {
		    ...
		    if (((String) activities.getSelectedItem())
			    .compareTo(healthInformation[1]) == 0) {
			if (Integer.parseInt(value.getText()) > 0
			    ...
			    notice.setText(new String(
				    "Error: Invalid Blood Sugar Levels"));
			}
		    }

		    // If the user is entering Heart Rate
		    if (((String) activities.getSelectedItem())
			    .compareTo(healthInformation[2]) == 0) {
			if (Integer.parseInt(value.getText()) > 0
